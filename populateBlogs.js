...
const authors = [
    'Shapesway AI Team',
    'Shapesway DevOps',
    'Shapesway Engineering',
    'Shapesway Security',
    'Shapesway Mobile Tech',
    'Shapesway Product Team',
    'Shapesway Growth Architects',
    'Shapesway SEO Experts',
    'Shapesway Ad Ops',
    'Shapesway Innovation',
    'Shapesway Frontend'
];
...
existingBlogs.forEach(post => {
    if (categoryCounts[post.category] !== undefined) {
        categoryCounts[post.category]++;
    }
});

let currentDate = new Date(2026, 2, 4); // March 4, 2026

categories.forEach(category => {
    const needed = 15 - categoryCounts[category];
    for (let i = 0; i < needed; i++) {
        const index = categoryCounts[category] + 1 + i;
        const title = \`Insightful Guide on \${category} - Part \${index}\`;
        const slug = generateSlug(title);
        
        currentDate.setDate(currentDate.getDate() - Math.floor(Math.random() * 3) - 1);
        const dateStr = currentDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: '2-digit' });

        existingBlogs.push({
            id: globalIdCounter++,
            slug: slug,
            title: \`Exploring \${category} Trends: Volume \${index}\`,
            date: dateStr,
            category: category,
            author: getRandomItem(authors),
            excerpt: \`An in-depth look at the latest methodologies, strategies, and industry shifts within the realm of \${category}, providing actionable insights for growth and scalability.\`,
            image: getRandomItem(images),
            content: \`
            <h2>Understanding the Fundamentals of \${category}</h2>
            <p>In our modern digital era, mastering <strong>\${category}</strong> has become more critical than ever. Organizations leading the charge are constantly innovating their approaches, driving significant value across the entire enterprise ecosystem.</p>
            
            <h3>Core Strategies and Implementations</h3>
            <p>This discussion focuses on optimizing workflows and expanding capabilities using state-of-the-art frameworks. We examine the core tenets of \${category} and detail the exact technical implementations that yield high performance and resilience.</p>
            <ul>
                <li><strong>Scalability:</strong> Ensuring infrastructure and design scales autonomously.</li>
                <li><strong>Performance:</strong> Reducing latency and ensuring quick resolution times.</li>
                <li><strong>Compliance and Security:</strong> Operating under modern privacy regulations.</li>
            </ul>

            <h3>The Road Ahead</h3>
            <p>Embracing a forward-looking mindset in \${category} dictates the winners of tomorrow. As technologies evolve, remaining agile and well-informed is key to long-term sustainability.</p>
            \`
        });
    }
});

// Build the array contents manually as string, avoiding double escaping content
let exportStr = "export const blogPosts = [\\n";
existingBlogs.forEach((post, i) => {
    exportStr += "    {\\n";
    exportStr += \`        id: \${post.id},\\n\`;
    exportStr += \`        slug: '\${post.slug}',\\n\`;
    exportStr += \`        title: '\${post.title}',\\n\`;
    exportStr += \`        date: '\${post.date}',\\n\`;
    exportStr += \`        category: '\${post.category}',\\n\`;
    exportStr += \`        author: '\${post.author}',\\n\`;
    exportStr += \`        excerpt: "\${post.excerpt.replace(/"/g, '\\\\"')}",\\n\`;
    exportStr += \`        image: '\${post.image}',\\n\`;
    exportStr += \`        content: \\\`\${post.content}\\\`\\n\`;
    exportStr += "    }";
    if (i < existingBlogs.length - 1) exportStr += ",\\n";
    else exportStr += "\\n";
});
exportStr += "];\\n";

fs.writeFileSync('C:\\\\Users\\\\Admin\\\\Desktop\\\\my project\\\\Shapesway website\\\\src\\\\data\\\\blogData.js', exportStr);
console.log('Successfully wrote 15 posts per category to blogData.js.');
